 ...
 *
 * cwd is the package root so the bare specifier `@moe/ide-adapter-contract`
 * resolves through this package's own `exports` map via Node's self-reference
 * rule — the exact resolution the adapter in task-9fd52b41 will get.
 */
...
try {
  ...
  report({
    ...
    layerType: typeof ns.IDE_ADAPTER_LAYER,
    reasonCodeCount: Array.isArray(ns.IDE_ADAPTER_REASON_CODES)
      ? ns.IDE_ADAPTER_REASON_CODES.length
      : -1,
    decisionExports: ["decideDaemonDiscovery", "decideDaemonStart", "decideControlRoomOpen"]
      .filter((key) => typeof ns[key] === "function").length,
  });
} catch (error) {
...
/**
 * Calls the contract through the bare specifier in plain Node. Type-stripping
 * erases every interface, so this is the only proof that a REAL value survives
 * to the runtime surface rather than the namespace being decoratively non-empty.
 */
const REPORT_ROOT_DECISION = `
const report = (value) => process.stdout.write(JSON.stringify(value));
try {
  const { decideDaemonStart } = await import("@moe/ide-adapter-contract");
  report({ outcome: "IMPORTED", result: decideDaemonStart({ detail: "no listener", status: "LAUNCHED_UNCONFIRMED" }) });
} catch (error) {
  report({ outcome: "FAILED", code: error.code ?? "NO_CODE" });
}
`;
...
it("loads @moe/ide-adapter-contract in Node's strip-types runtime with its values defined", async () => {
  ...
  // cleanly and only fails at first use. The counts are pinned, not merely
  // non-zero, so silently dropping a value export from the runtime surface reddens.
...
it("decides through the bare specifier in plain Node, failing closed on unconfirmed listening", async () => {
  expect(await probe(REPORT_ROOT_DECISION)).toEqual({
    outcome: "IMPORTED",
    result: {
...
it("still refuses an unbridged test module with ERR_MODULE_NOT_FOUND", async () => {
  ...
  expect(await probe(reportUnbridged("./src/ide-adapter-contract.test.js"))).toEqual({
...
it("excludes every test module for a named reason, and only those", () => {
  ...
  expect(verdicts).toEqual({
    "ide-adapter-contract.test.ts": "test-file",
    "ide-adapter-runtime-entrypoint.test.ts": "test-file",
  });
});
