...
import {
  ...
  type LinuxBoundaryPayloads,
  type LinuxClassificationContext,
} from "./linux-boundary.js";
import { isFactAgeMs } from "./linux-facts.js";
import {
  PLATFORM_BOUNDARIES,
...
/**
 * The Linux platform observation seam.
 *
 ...
 * answer, because no process here has looked at a real Linux host.
 *
 * `platform/linux/` is deliberately NOT created: that directory belongs to the
 * Linux effect-conformance task, which is the consumer of these verdicts.
 */
export { PLATFORM_LINUX_LAYER, classifyLinuxBoundary } from "./linux-boundary.js";
export type {
...
/** The `host.os` value this adapter, and only this adapter, speaks for. */
export const PLATFORM_LINUX_OS = "linux" as const;

export const LINUX_SUPPORTED_ARCHITECTURES = Object.freeze(["x64", "arm64"] as const);
export type LinuxSupportedArchitecture = (typeof LINUX_SUPPORTED_ARCHITECTURES)[number];
 ...
 * first.
 */
export type LinuxBoundaryFacts = {
  readonly [Boundary in PlatformBoundary]: PlatformFactEnvelope<
    LinuxBoundaryPayloads[Boundary]
  > | null;
};

export interface ObserveLinuxPlatformInput {
  readonly host: PlatformHostIdentity;
  readonly asOf: string;
  readonly maxFactAgeMs: number;
  readonly facts: LinuxBoundaryFacts;
}
 ...
 * a Linux verdict about a win32 fact would be meaningless whatever the fact
 * says. Shape failures answer as PLATFORM_CONTRACT and host/architecture
 * failures as PLATFORM_LINUX, so a caller can tell "you called this wrong" from
 * "this does not hold here".
 */
export function observeLinuxPlatform(input: unknown): PlatformObservation {
  ...
  if (!isCanonicalUtcTimestamp(asOf) || !isFactAgeMs(maxFactAgeMs)) {
  ...
  if (host.os !== PLATFORM_LINUX_OS) {
    return refuseEvery(host, "PLATFORM_HOST_MISMATCH", PLATFORM_LINUX_LAYER, "the declared host is not a Linux host, so no Linux fact can be proven about it");
  }
  if (!isSupportedArchitecture(host.arch)) {
    return refuseEvery(host, "PLATFORM_ARCH_UNSUPPORTED", PLATFORM_LINUX_LAYER, "this adapter does not classify facts on the declared architecture");
  }
  const facts = snapshotExactRecord(snapshot["facts"], PLATFORM_BOUNDARIES);
  if (facts === null) {
    return refuseEvery(host, "PLATFORM_COVERAGE_INCOMPLETE", PLATFORM_LINUX_LAYER, "facts does not name every platform boundary exactly once");
  }
  const context: LinuxClassificationContext = { host, asOf, maxFactAgeMs };
  return observation(
    host,
...
function boundaryVerdict(
  ...
  const result = classifyLinuxBoundary(boundary, envelope, context);
...
function isSupportedArchitecture(value: string): value is LinuxSupportedArchitecture {
  return (LINUX_SUPPORTED_ARCHITECTURES as readonly string[]).includes(value);
}
