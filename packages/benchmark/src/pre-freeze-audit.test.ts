...
const HAS_EXPLICIT_PIN_ROOT = (original?.trim().length ?? 0) > 0;
afterEach(() => {
...
describe("pre-freeze audit entry point (task-71a4fac5d15044c08f6617f50a561e39)", () => {
  it.runIf(HAS_EXPLICIT_PIN_ROOT)("passes the two pinned documents end to end", () => {
  ...
  it.runIf(HAS_EXPLICIT_PIN_ROOT)(
    "sums every half's case count, and every half generated cases", () => {
    const report = runPreFreezeAudit();
  ...
  it.runIf(HAS_EXPLICIT_PIN_ROOT)(
    "agrees with the pure form given the same already-verified sources", () => {
  ...
  it("refuses rather than passing when a pinned document cannot be read", () => {
    process.env[PINNED_DOCUMENT_ROOT_ENV] = join(
      tmpdir(), `moe-pinned-root-does-not-exist-${String(process.pid)}`,
    );
    const report = runPreFreezeAudit();
