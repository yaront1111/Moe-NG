...
export function devProvenance(ref, version = 3) {
    return {
        sourceFactRef: ref,
...
export function devFact(code, confidence, overrides = {}) {
    return {
        code,
        confidence,
...
export function devAllTrueFacts() {
...
export function devFactsWith(code, replacement) {
    const facts = devAllTrueFacts().filter((entry) => entry["code"] !== code);
    return replacement === null ? facts : [...facts, replacement];
}
export function devBundle(nodeKey, facts = devAllTrueFacts(), overrides = {}) {
    return {
        nodeKey,
...
export function devBundles() {
    return [
        devBundle(DEV_READY),
        devBundle(DEV_RESOURCE, devFactsWith("READINESS_RESOURCE_CONFIRMED_ACTIVE", devFact("READINESS_RESOURCE_CONFIRMED_ACTIVE", "CONFIRMED_FALSE"))),
        devBundle(DEV_CAPABILITY, devFactsWith("READINESS_CAPABILITY", devFact("READINESS_CAPABILITY", "CONFIRMED_FALSE"))),
        devBundle(DEV_ADVISORY),
        devBundle(DEV_DONE),
    ];
}
export function devEdgeFacts(state = "UNSATISFIED", overrides = {}) {
    return ["dev-edge-ready", "dev-edge-resource", "dev-edge-capability"].map((edgeKey) => ({
        edgeKey,
        state: overrides[edgeKey] ?? state,
    }));
}
export function devInput(overrides = {}) {
    return { hardEdgeFacts: devEdgeFacts(), nodeFacts: devBundles(), ...overrides };
}
/** Replace one node's bundle in the base set. */
export function devBundlesWith(nodeKey, bundle) {
 ...
 */
export function devWait(deadlineGate = "REVIEW_QUALIFICATION", recheckAtGate = "EXECUTOR_CLAIM") {
    return {
        waitRef: "wait:dev",
...
export const DEV_CHAIN_SNAPSHOT = {
    completionNodeKey: DEV_DONE,
    nodes: [
