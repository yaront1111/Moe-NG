 ...
 *
 * Detection performs one precomputed descendant-bitset membership operation per
 * producer out-arc; bigint cost grows with the bounded graph bit width. Witness
 * reconstruction is a bounded BFS that runs only for a candidate. Results are
 * sorted by `edgeKey` for determinism.
 */
