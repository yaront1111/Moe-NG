...
describe("analyzeGraphStructure — single execution node (required test 1)", () => {
  it("reports stage count 1 and all frontier widths as 1", () => {
    ...
    expect(analysis.logicalReadyWidth).toBe(1);
    expect(analysis.admissionReadyWidth).toBe(1);
    expect(analysis.dispatchableWidth).toBe(1);
    expect(analysis.structuralCriticalPathNodeKeys).toEqual(["dev-solo"]);
    expect(analysis.durationEstimate).toBeNull();
  });

  it("frontier widths are null (UNKNOWN, never 0) without frontier facts", () => {
    const graph = mustValidate(
      devSnapshot([devNode("dev-solo")], [], "dev-solo"),
    );
    const analysis = analyzeGraphStructure(graph);
    expect(analysis.logicalReadyWidth).toBeNull();
    expect(analysis.admissionReadyWidth).toBeNull();
    expect(analysis.dispatchableWidth).toBeNull();
  });
});
...
describe("analyzeGraphStructure — fan-in join (required test 3)", () => {
  it("reports ready width 3 and stage count 2", () => {
    ...
    const analysis = analyzeGraphStructure(graph, frontier);
    expect(analysis.admissionReadyWidth).toBe(3);
...
describe("analyzeGraphStructure — frontier width truth", () => {
  it("never mixes logical, admission, and dispatchable widths", () => {
    const snapshot = devChain(3);
    const graph = mustValidate(snapshot);
    const frontier = mustPartition(graph, {
      hardEdgeFacts: devUniformEdgeFacts(snapshot, "SATISFIED"),
      nodeAvailabilityFacts: [
        { admissionEligible: false, dispatchAvailable: true, nodeKey: "dev-node-1" },
        { admissionEligible: true, dispatchAvailable: false, nodeKey: "dev-node-2" },
        { admissionEligible: true, dispatchAvailable: true, nodeKey: "dev-node-3" },
      ],
    });
    const analysis = analyzeGraphStructure(graph, frontier);
    expect(analysis.logicalReadyWidth).toBe(3);
    expect(analysis.admissionReadyWidth).toBe(2);
    expect(analysis.dispatchableWidth).toBe(1);
  });
});
...
describe("analyzeGraphStructure — fail-closed on a forged precondition", () => {
  it("rejects a cast cyclic graph before attempting structural analysis", () => {
    // A cast cannot manufacture the opaque runtime provenance established by
    // validateGraphSnapshot, so malformed/cyclic structures fail at the API
    // boundary rather than reaching a partial analysis.
    const cyclic = {
    ...
    expect(() => analyzeGraphStructure(cyclic)).toThrow(
