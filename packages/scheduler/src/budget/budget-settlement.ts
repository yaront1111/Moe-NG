/**
 * Settlement and reconciliation (design 11.1 601-613, 11.4 655-664, recovery matrix 1038-1039). An
 * exact correlated receipt commits RESERVED -> COMMITTED and refunds only the proven remainder;
 * PARTIAL commits its lower bound and quarantines the rest; UNKNOWN or a missing receipt quarantines
 * the whole hold rather than laundering it into zero measured spend. Measured use above the hold
 * enters COMMITTED and is simultaneously recorded as explicit overrun with state OVERDRAWN, which is
 * what keeps AVAILABLE+RESERVED+QUARANTINED+COMMITTED = authorized + recorded overrun; no bucket goes
 * negative and no sibling meter funds another, and quarantine keeps only the four closed exits.
 * RESERVATION_STATES is closed and unowned here, so settlement mints its own record. Pure: no Date,
 * no randomness, no registry — a replay is fenced by the caller-supplied prior plus the version fence,
 * so the store, not this module, enforces at-most-one. SETTLING is a ledger-level lifecycle needing
 * account-wide knowledge, and close can only judge the settlement records it is handed.
 */
...
export const BUDGET_SETTLEMENT_ISSUE_CODES = Object.freeze([
  "BUDGET_SETTLEMENT_ACKNOWLEDGEMENT_MISSING", "BUDGET_SETTLEMENT_ALREADY_SETTLED",
  "BUDGET_SETTLEMENT_COUNTER_EXHAUSTED", "BUDGET_SETTLEMENT_DUPLICATE_METER",
  "BUDGET_SETTLEMENT_EVIDENCE_AMBIGUOUS", "BUDGET_SETTLEMENT_IDENTITY_MISMATCH",
  "BUDGET_SETTLEMENT_ILLEGAL_CLOSE", "BUDGET_SETTLEMENT_INSUFFICIENT_QUARANTINED",
  "BUDGET_SETTLEMENT_INSUFFICIENT_RESERVED", "BUDGET_SETTLEMENT_MALFORMED",
  "BUDGET_SETTLEMENT_MEASUREMENT_CONFLICT", "BUDGET_SETTLEMENT_NOT_ACTIVATED",
  "BUDGET_SETTLEMENT_SEQUENCE_STALE", "BUDGET_SETTLEMENT_STALE_VERSION",
  "BUDGET_SETTLEMENT_UNCORRELATED_MEASUREMENT", "BUDGET_SETTLEMENT_UNKNOWN_METER"] as const);
export const SETTLEMENT_STATES = Object.freeze(["QUARANTINED", "SETTLED", "WRITTEN_OFF"] as const);
export const LINE_DISPOSITIONS = Object.freeze(["EXACT", "LOWER_BOUND", "UNKNOWN_HELD",
  "CONSERVATIVE_WRITE_OFF", "NEVER_STARTED_REFUND"] as const);
...
export interface SettlementLine { readonly meter: string; readonly reserved: number; readonly committed: number;
  readonly refunded: number; readonly quarantined: number; readonly disposition: LineDisposition;
  readonly identity: string | null; readonly sequence: number | null }
export interface SettlementRecord { readonly settlementId: string; readonly reservationId: string;
  readonly accountId: string; readonly attemptRef: string; readonly version: number;
  readonly state: SettlementState; readonly lines: readonly SettlementLine[];
...
export interface ReconcileEvidence { readonly measurements: readonly NormalizedMeasurement[] | null;
  readonly neverStartedProofRef: string | null }
export interface SettleCommand { readonly expectedViewVersion: number;
  readonly expectedReservationVersion: number; readonly prior: SettlementRecord | null }
...
const AMOUNTS = ["available", "reserved", "quarantined", "committed"] as const;
const LINE_AMOUNTS = ["reserved", "committed", "refunded", "quarantined"] as const;
const HOLD_KEYS = ["purpose", "meter", "quantity"] as const;
const SETTLE_KEYS = ["expectedViewVersion", "expectedReservationVersion", "prior"] as const;
const FENCE_KEYS = ["expectedViewVersion", "expectedSettlementVersion"] as const;
const ACK_KEYS = [...FENCE_KEYS, "acknowledgementRef", "enforceableUpperBound"] as const;
const ARM_KEYS = ["measurements", "neverStartedProofRef"] as const;
...
const RESERVATION_KEYS = ["reservationId", "accountId", "admissionRef", "state", "version", "lines",
  "attemptRef", "neverStartedProofRef"] as const;
const SETTLEMENT_KEYS = ["settlementId", "reservationId", "accountId", "attemptRef", "version", "state",
  "lines", "unknownExternalLiability", "overrun"] as const;
...
const issue = (code: BudgetSettlementIssueCode): BudgetSettlementIssue =>
  ({ code, message: code.slice("BUDGET_SETTLEMENT_".length).toLowerCase().split("_").join(" ") });
...
const accept = (settlement: SettlementRecord | null, view: BudgetAvailableView): BudgetSettlementResult =>
  deepFreeze({ ok: true, settlement, view });
...
const bucketOf = (view: BudgetAvailableView, meter: string): BudgetMeterBuckets | undefined =>
  view.meters.find((entry) => entry.meter === meter);
...
const fence = (view: BudgetAvailableView, version: number, expectedView: number, expected: number): readonly Check[] => [
  [view.version >= MAX_BUDGET_VERSION, "BUDGET_SETTLEMENT_COUNTER_EXHAUSTED"],
  [view.version !== expectedView || version !== expected, "BUDGET_SETTLEMENT_STALE_VERSION"]];
...
const isView = (value: unknown): value is BudgetAvailableView =>
  ...
  && isPlainArray(value.meters) && value.meters.length > 0 && value.meters.length <= MAX_LINES
  && value.meters.every((e) => isPlainRecord(e) && isRef(e.meter) && AMOUNTS.every((k) => isCount(e[k])));
...
function readHolds(value: unknown): Map<string, number> | null {
  ...
  for (const entry of value) {
    const item = readRecord(entry, HOLD_KEYS);
    if (item === null || !isRef(item.meter) || !isCount(item.quantity)) return null;
    totals.set(item.meter, (totals.get(item.meter) ?? 0) + item.quantity); }
  return totals; }
...
function readReadings(value: unknown): Reading[] | null {
  ...
  for (const entry of value) { const one = readMeasurement(entry); if (one === null) return null; output.push(one); }
  return output; }
interface Movement { available: number; reserved: number; quarantined: number; committed: number }
const stateOf = (lines: readonly SettlementLine[]): SettlementState =>
  lines.every((l) => l.quarantined === 0 && RESOLVED.includes(l.disposition)) ? "SETTLED" : "QUARANTINED";
function shift(view: BudgetAvailableView, moves: ReadonlyMap<string, Movement>, state: BudgetAccountState): BudgetAvailableView {
  return { accountId: view.accountId, state, version: view.version + 1,
    meters: view.meters.map((e) => { const m = moves.get(e.meter);
      return m === undefined ? { ...e } : { meter: e.meter, available: e.available + m.available,
        reserved: e.reserved + m.reserved, quarantined: e.quarantined + m.quarantined,
        committed: e.committed + m.committed }; }) }; }
/**
 * One meter. `hold` sits in bucket `from` and `credited` is what this meter already committed, so a
 * receipt quantity is always TOTAL measured use and the committed delta is quantity - credited.
 * UNKNOWN and a missing receipt both hold the units instead of reporting zero measured spend.
 */
function dispose(meter: string, hold: number, credited: number, from: "reserved" | "quarantined",
  ...
  move[from] -= hold;
  if (reading === undefined || !reading.known) {
    move.quarantined += hold;
    return { line: { ...base, committed: credited, refunded: 0, quarantined: hold, disposition: "UNKNOWN_HELD" },
      move, overrun: 0 }; }
  const delta = reading.quantity - credited, rest = Math.max(0, hold - delta);
  move.committed += delta;
  move[reading.complete ? "available" : "quarantined"] += rest;
  return { line: { ...base, committed: reading.quantity, refunded: reading.complete ? rest : 0,
    quarantined: reading.complete ? 0 : rest, disposition: reading.complete ? "EXACT" : "LOWER_BOUND" },
  move, overrun: Math.max(0, delta - hold) }; }

/**
 * Settles one ACTIVATED reservation. `prior` is the settlement the caller already holds for it:
 * byte-identical inputs return THAT record by reference and move nothing, while any other prior
 * refuses, so a replay can never double-commit.
 */
export function settleReservation(view: BudgetAvailableView, reservation: ReservationRecord,
  ...
  const held = record === null ? null : readHolds(record.lines);
  const envelope = readRecord(evidence, ["measurements"]);
  const readings = envelope === null ? null : readReadings(envelope.measurements);
  if (!isView(view) || cmd === null || record === null || held === null || readings === null
    ...
    || (cmd.prior !== null && !isPlainRecord(cmd.prior)) || !isRef(record.reservationId)
    || !isRef(record.accountId) || !isRef(record.admissionRef) || !isCount(record.version)) {
    return fail(view, null, "BUDGET_SETTLEMENT_MALFORMED"); }
  const attemptRef = record.attemptRef;
  const code = firstOf([
    [record.accountId !== view.accountId
      || record.reservationId !== deriveReservationId(record.accountId, record.admissionRef),
    "BUDGET_SETTLEMENT_IDENTITY_MISMATCH"],
    [record.state !== "ACTIVATED" || !isRef(attemptRef), "BUDGET_SETTLEMENT_NOT_ACTIVATED"],
    ...fence(view, record.version, cmd.expectedViewVersion, cmd.expectedReservationVersion),
    [readings.some((r) => r.run !== attemptRef), "BUDGET_SETTLEMENT_UNCORRELATED_MEASUREMENT"],
  ...
  for (const [meter, hold] of held) {
    ...
    lines.push(one.line);
    moves.set(meter, one.move);
    if (one.overrun > 0) overrun.push({ meter, amount: one.overrun }); }
  const settlement: SettlementRecord = { settlementId: deriveSettlementId(record.reservationId),
    reservationId: record.reservationId, accountId: record.accountId, attemptRef: attemptRef as string,
    version: 0, state: stateOf(lines), lines, unknownExternalLiability: false, overrun };
  if (cmd.prior !== null) {
    return JSON.stringify(cmd.prior) === JSON.stringify(settlement)
      ? accept(cmd.prior as unknown as SettlementRecord, view)
      : fail(view, null, "BUDGET_SETTLEMENT_ALREADY_SETTLED"); }
...
function openQuarantine(view: BudgetAvailableView, settlement: unknown, cmd: Record<string, unknown> | null,
  ...
  const code = firstOf([
    [record.state !== "QUARANTINED", "BUDGET_SETTLEMENT_ALREADY_SETTLED"],
    ...extra(record),
    ...fence(view, record.version, cmd.expectedViewVersion, cmd.expectedSettlementVersion),
    [record.lines.some((l) => (bucketOf(view, l.meter)?.quarantined ?? -1) < l.quarantined),
      "BUDGET_SETTLEMENT_INSUFFICIENT_QUARANTINED"],
  ]);
  return code === null ? { record } : { code }; }

/**
 * Design exits 1 and 2, one verb with two evidence arms: exactly one of a correlated receipt or a
 * never-started proof. COMMITTED is never touched by the proof arm, so an already committed
 * attempt.count stays spent, and a receipt below the committed lower bound is a conflict, not a refund.
 */
export function reconcileSettlement(view: BudgetAvailableView, settlement: SettlementRecord,
  ...
  const readings = arm === null || arm.measurements === null ? null : readReadings(arm.measurements);
  if (arm === null || (arm.measurements !== null && readings === null) || (proof !== null && !isRef(proof))) {
  ...
  const at = (r: SettlementRecord, m: string): SettlementLine | undefined => r.lines.find((l) => l.meter === m);
  const opened = openQuarantine(view, settlement, readRecord(command, FENCE_KEYS), (r) => [
    ...
    [readings !== null && new Set(readings.map((x) => x.meter)).size !== readings.length,
      "BUDGET_SETTLEMENT_DUPLICATE_METER"],
    ...
    [some((x) => x.known && x.quantity < (at(r, x.meter)?.committed ?? 0)) || (readings === null
      && r.lines.some((l) => l.committed > 0)), "BUDGET_SETTLEMENT_MEASUREMENT_CONFLICT"],
  ]);
  if ("code" in opened) return fail(view, settlement, opened.code);
  const record = opened.record, moves = new Map<string, Movement>(), lines: SettlementLine[] = [];
  const overrun = [...record.overrun];
  for (const line of record.lines) {
    ...
    if (readings === null) {
      ...
      lines.push({ ...line, refunded: line.refunded + line.quarantined, quarantined: 0,
        disposition: "NEVER_STARTED_REFUND" });
      continue; }
...
/**
 * Design exits 3 and 4. Recovery matrix 1039 requires human acknowledgement PLUS a bound
 * classification, so both are one evidence requirement here; no policy is re-evaluated. The whole
 * held reservation commits and nothing refunds, and an unenforceable bound mints persistent liability.
 */
export function conservativeSettle(view: BudgetAvailableView, settlement: SettlementRecord,
  command: ConservativeCommand): BudgetSettlementResult {
  const cmd = readRecord(command, ACK_KEYS);
  const opened = openQuarantine(view, settlement, cmd, () => cmd === null ? [] : [
    [!isRef(cmd.acknowledgementRef) || typeof cmd.enforceableUpperBound !== "boolean",
      "BUDGET_SETTLEMENT_ACKNOWLEDGEMENT_MISSING"]]);
  if ("code" in opened) return fail(view, settlement, opened.code);
  const record = opened.record, moves = new Map<string, Movement>();
  const lines = record.lines.map((l) => {
    if (l.quarantined === 0) return l;
    moves.set(l.meter, { available: 0, reserved: 0, quarantined: -l.quarantined, committed: l.quarantined });
    return { ...l, committed: l.committed + l.quarantined, quarantined: 0,
      disposition: "CONSERVATIVE_WRITE_OFF" as const }; });
  return accept({ ...record, version: record.version + 1, state: "WRITTEN_OFF", lines,
    unknownExternalLiability: record.unknownExternalLiability || cmd?.enforceableUpperBound === false },
  shift(view, moves, view.state));
}

/**
 * Design 664: no account closes with RESERVED or QUARANTINED units, and AVAILABLE must already have
 * been returned to the parent by the ledger. OVERDRAWN has no close exit, so it refuses. The terminal
 * state is structural: any supplied settlement carrying unknown liability closes the fourth way.
 */
export function closeSettledView(view: BudgetAvailableView, settlements: readonly SettlementRecord[],
  command: CloseCommand): BudgetSettlementResult {
  const cmd = readRecord(command, ["expectedVersion"]);
  const records = isPlainArray(settlements) && settlements.length <= MAX_LINES
    ? settlements.map(readSettlement) : null;
  ...
  if (code !== null) return fail(view, null, code);
  return accept(null, shift(view, new Map(), records.some((r) => r?.unknownExternalLiability === true)
    ? "CLOSED_WITH_UNKNOWN_LIABILITY" : "CLOSED"));
}
