...
describe("BLOCKER 1 — non-execution nodes are excluded from the frontier", () => {
  ...
  it("counts only executable nodes in every frontier width", () => {
    const graph = mustValidate(snapshot);
    ...
    if (frontier.ok) {
      ...
      expect(analysis.logicalReadyWidth).toBe(2);
      expect(analysis.admissionReadyWidth).toBe(2);
      expect(analysis.dispatchableWidth).toBe(2);
    }
  });
});
...
describe("MAJOR 2 — frontier partition is bound to its graph identity", () => {
  ...
  it("rejects analysing graph A with graph B's frontier partition", () => {
    ...
    if (partitionB.ok) {
      ...
      ).toThrow(/FRONTIER_VALIDATION_PROVENANCE_INVALID/);
    }
  });
...
describe("MAJOR 4 — total-edge policy cap (provisional 64, separate from hard cap)", () => {
  const producerCount = 23;

  function mixedEdges(total: number): GraphEdge[] {
    // Reuse a bounded producer set so the total-edge boundary does not require
    // raising the independent node policy. Every producer first gets one HARD
    // path to completion; later relations mix HARD and ADVISORY kinds.
    const edges: GraphEdge[] = [];
    for (let i = 0; i < total; i += 1) {
      const key = `dev-e${i}`;
      const producer = `dev-p${i % producerCount}`;
      edges.push(
        i < producerCount || i % 2 === 0
          ? devHardEdge(key, producer, "dev-done")
          : devAdvisoryEdge(key, producer, "dev-done"),
      );
    }
    return edges;
  }
  function mixedNodes(): GraphSnapshot["nodes"] {
    const nodes = [devNode("dev-done", true)];
    for (let i = 0; i < producerCount; i += 1) {
      nodes.push(devNode(`dev-p${i}`));
    }
    return nodes;
  }
  ...
  it(`accepts exactly ${DEFAULT_MAX_TOTAL_EDGES} total edges`, () => {
    ...
    // Keep the hard-edge cap out of the way; only exercise total-edge policy.
    const result = validateGraphSnapshot(snapshot, {
      maxHardEdges: total,
    });
    expect(result.ok).toBe(true);
