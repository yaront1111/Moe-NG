 ...
 *
 * cwd is the package root so the bare specifier `@moe/skills` resolves through
...
try {
  const ns = await import("@moe/skills");
  ...
  report({
    ...
    loadSkillBundle: typeof ns.loadSkillBundle,
    validateSkillManifestBytes: typeof ns.validateSkillManifestBytes,
    resolveSkillSnapshot: typeof ns.resolveSkillSnapshot,
    toSkillRendererInput: typeof ns.toSkillRendererInput,
    manifestVersion: typeof ns.SKILL_MANIFEST_VERSION,
    errorCodesFrozen: Object.isFrozen(ns.SKILL_ERROR_CODES),
  });
} catch (error) {
...
it("loads @moe/skills in Node's strip-types runtime with its named exports defined", async () => {
  ...
  // cleanly and only fails at first use. `toSkillRendererInput` arrives through
  // skill-snapshot -> skill-loader -> skill-contract, the deepest chain here.
  expect(await probe(REPORT_ROOT_ENTRY)).toEqual({
    outcome: "IMPORTED",
    hasNamedExports: true,
    undefinedBindingCount: 0,
    loadSkillBundle: "function",
    validateSkillManifestBytes: "function",
    resolveSkillSnapshot: "function",
    toSkillRendererInput: "function",
    manifestVersion: "string",
    errorCodesFrozen: true,
  });
}, CHILD_TIMEOUT_MS);
...
it("still refuses an unbridged test module with ERR_MODULE_NOT_FOUND", async () => {
  ...
  expect(await probe(reportUnbridged("./src/skill-manifest.test.js"))).toEqual({
    outcome: "FAILED",
...
it("bridges exactly the non-test modules and nothing else", () => {
...
it("excludes every test module for a named reason, and only those", () => {
  ...
  expect(verdicts).toEqual({
    "skill-bundle.test.ts": "test-file",
    "skill-manifest.test.ts": "test-file",
    "skills-runtime-entrypoint.test.ts": "test-file",
  });
});
...
it("bridges every module reachable from the package entry point", () => {
  ...
  // More than the entry alone, or the closure walked past a hole and stopped.
