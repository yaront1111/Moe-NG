  ...
  it("lists an escalation per escalation.decide offer, named through the runs read", () => {
    ...
    const runs: RunsOutcome = {
      goals: [{
        ...
        nodes: [{
          accepted: null, claim: null, criterionIds: [], declaredMigrations: null, dependsOn: [], lastActivityAt: null, nodeKey: "node-x", nodeRef: "execution-node-x",
    ...
    expect(data.items[0]?.detail).toBe("o failed review 3 times (last: rejected: same finding again). Allow one more attempt, or replan the work into a successor goal that carries these findings.");
