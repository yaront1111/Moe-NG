...
/**
 * GATE 1 on the V1 command plane. The daemon states which plane it serves on
 * `/bootstrap`; while that is V1 - every real installation until
 * `cutover.activate` commits - agents propose Product Contract revisions on
 * the `/1` plane and the pending revision lives behind
 * `/product-contract/pending/read`. This module is that plane's card wire:
 * the V2 sibling (gate1-approval.ts) reads `/v2/...` and renders the `/2`
 * revision model; cordum-app picks one by the stated plane, never by build.
 *
...
export const GATE1_V1_PENDING_READ_PATH = "/product-contract/pending/read" as const;
const REQUEST_TIMEOUT_MS = 15_000;

export interface Gate1RequirementViewV1 {
  readonly requirementId: string;
  readonly statement: string;
}
export interface Gate1CriterionViewV1 {
  readonly criterionId: string;
  readonly statement: string;
}

export interface Gate1ClarificationViewV1 {
...
export interface Gate1PendingViewV1 {
  ...
  readonly clarifications: readonly Gate1ClarificationViewV1[];
  readonly contractId: string;
  readonly criteria: readonly Gate1CriterionViewV1[];
  readonly requirements: readonly Gate1RequirementViewV1[];
  readonly revisionDigest: string;
  readonly revisionId: string;
  readonly status: "PENDING";
}

export type Gate1ReadOutcomeV1 =
  | Gate1PendingViewV1
...
function errored(code: string): Gate1ReadOutcomeV1 {
...
function clarificationsOf(value: unknown): readonly Gate1ClarificationViewV1[] | null {
  if (value === undefined) return [];
  if (!Array.isArray(value)) return null;
  const rows: Gate1ClarificationViewV1[] = [];
  for (const raw of value) {
...
export function mapGate1AnswerV1(status: number, response: unknown): Gate1ReadOutcomeV1 {
  ...
  let approval: Gate1PendingViewV1["approval"] = null;
  if (approvalRaw !== null) {
...
export async function readPendingContractV1(
  headers: Readonly<Record<string, string>>,
  goalId: string,
  post?: (body: string) => Promise<Response>,
): Promise<Gate1ReadOutcomeV1> {
  const send = post ?? ((body: string): Promise<Response> => fetch(GATE1_V1_PENDING_READ_PATH, {
...
function answerOf(response: unknown, commandId: string): Gate1ApprovalOutcomeV1 {
...
export function createGate1ApprovalPortV1(wire: Gate1ApprovalWireV1): Gate1ApprovalPortV1 {
  const send = async (
    kind: typeof GATE1_ANSWER_COMMAND_KIND | typeof GATE1_COMMAND_KIND,
  ...
  return Object.freeze({
    answer: async (
      ...
      return send(GATE1_ANSWER_COMMAND_KIND, affordance, {
        answerProjectionDigest: chosen.projectionDigest,
        clarificationId: clarification.clarificationId,
        contractId,
      });
    },
    submit: async (pending: Gate1PendingViewV1): Promise<Gate1ApprovalOutcomeV1> => {
