...
import { createGate1ApprovalPortV1, mapGate1AnswerV1 } from "./gate1-v1-approval.js";
...
describe("mapGate1AnswerV1", () => {
  it("maps a PENDING frame with the daemon-minted approval verbatim", () => {
    const mapped = mapGate1AnswerV1(200, PENDING_BODY);
  ...
  it("maps NONE, refusals and drifted shapes to their own outcomes", () => {
    expect(mapGate1AnswerV1(200, { outcome: "NONE" })).toEqual({ status: "NONE" });
    expect(mapGate1AnswerV1(200, { code: "X", layer: "L", outcome: "REFUSED" }))
      .toEqual({ code: "X", layer: "L", status: "REFUSED" });
    expect(mapGate1AnswerV1(403, { code: "LISTENER_ORIGIN_INVALID", layer: "LISTENER" }))
    ...
    expect(mapGate1AnswerV1(200, drifted))
      .toEqual({ code: "GATE1_RESPONSE_INVALID", layer: "CONTROL_ROOM_GATE1", status: "ERROR" });
  });
});

describe("createGate1ApprovalPortV1", () => {
  const pending = mapGate1AnswerV1(200, PENDING_BODY) as Gate1PendingViewV1;
  ...
  it("presents exactly the daemon-minted identity plus a fresh issuedAt", async () => {
    ...
    const outcome = await createGate1ApprovalPortV1(wire as never).submit(pending);
  ...
  it("withholds submit while the template is withheld, and answers via the recorded digest", async () => {
    const { calls, wire } = wireCapture();
    const port = createGate1ApprovalPortV1(wire as never);
  ...
  it("carries a daemon refusal out at its own code and layer", async () => {
    ...
    expect(await createGate1ApprovalPortV1(wire as never).submit(pending)).toEqual({
