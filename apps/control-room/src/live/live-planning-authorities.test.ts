...
/**
 * THE PLANNING SIDECAR AND THE DISPATCH SEAM THAT READS IT, driven without a board.
 *
 * Salvaged from the v1 `live-board-dispatch.test.tsx` when the v1 LiveBoard was removed:
 * every arm here drove `planningPayloadFor`, `PLANNING_AUTHORITY_KINDS` or
 * `dispatchAffordance` directly and never rendered the board, so the behaviour they pin
 * (the daemon's per-run planning material bound to the exact parsed offer, the two
 * planning refusals under their own names, the decide-time budget commitment read off the
 * daemon) is still production's. The `boardMayDispatch` predicate and the rendered
 * dispatch controls went with the v1 board; v2's work board is read-only by contract and
 * v2 dispatch goes through the `v2/approvals/*` ports.
 */
...
/** The staffed-agent step: on the surface, never authored from the control room. */
const AGENT_KIND = "node.deliver";

/**
 * The budget-commitment reader, stubbed. It must be attached wherever an `approval.decide`
 * is exercised: `dispatchAffordance` fails CLOSED without one, because `record.budgetRef` is
 * the daemon's decide-time commitment and no browser can mint it.
 */
const COMMITMENT_REF = "4d".repeat(32);
const readsCommitment = (): Promise<{ ref: string; status: "COMMITMENT" }> =>
  Promise.resolve({ ref: COMMITMENT_REF, status: "COMMITMENT" as const });
 ...
 * proposal or an approval carries is read off this material for the OFFER's own run; the
 * control room mints none of it, so a fixture is the only place these bytes can come from in
 * a unit test.
 */
...
describe("the corpus this file sweeps", () => {
  it("is non-empty, holds the approval kind, and holds many others", () => {
    ...
    // And the payload source refuses the staffed agent's step outright.
    expect(payloadFor(AGENT_KIND, "node-code-1")).toBeNull();
  });
});

/**
 * WHAT THE DISPATCHER MAY HAND BACK for the kinds v2 still authors off `payloadFor` and
 * `DEV_PAYLOADS`: `v2/ops/agent-provider-port.ts` calls `payloadFor(kind, null)` and
 * `v2/ops/activation-port.ts` drives the activation chain, `policy.validate` included, off
 * `DEV_PAYLOADS`. Neither arm needs a board; both came over verbatim from the v1
 * `live-board-dispatch.test.tsx`.
 */
describe("what the dispatcher may hand back", () => {
 ...
 * caller's behalf: the selector under test is the only mechanism that can decide, and swapping it
  ...
  it("merges the READ ref into the record it hands the builder, for the OFFERED run", async () => {
    ...
    const report = await dispatchAffordance({
      ...
      readBudgetCommitment: (runId) => {
        asked.push(runId);
        return Promise.resolve({ ref: COMMITMENT_REF, status: "COMMITMENT" as const });
      },
      sessionCredential: "cred",
      transport: rec.transport as never,
      version: 2,
    ...
    expect(record["budgetRef"]).toBe(COMMITMENT_REF);
  });
...
describe("the offer's target decides the planning identity", () => {
  ...
  it("refuses an offer whose run the daemon bound to no goal, before builder or transport", async () => {
    const rec = recorder();
    // The map is PRESENT and readable; it simply does not bind this run. The caller
...
describe("the two planning bindings refuse under their own names", () => {
  ...
  it("refuses a caller-minted twin of an offer that really does carry material", async () => {
    ...
    // record itself, so no caller can hand the dispatch authority it was never given.
