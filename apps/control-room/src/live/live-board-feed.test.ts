...
/**
 * THE SURFACE READER AND THE POLL LOOP, driven without a board.
 *
 * Salvaged from the v1 `live-board.test.tsx` when the v1 LiveBoard was removed: every arm
 * here calls `frameOfSurface` or `createBoardFeed` directly and never rendered the board,
 * and both remain v2 production - `use-workspace-surface.ts` and
 * `v2/approvals/live-needs-you.tsx` poll through `createBoardFeed`; `live-goals.tsx`,
 * `live-work-board.tsx`, `policy-install-port.ts` and `live-goal-create.ts` read through
 * `frameOfSurface`. What is pinned: the daemon's answer copied verbatim or refused whole as
 * LIVE_SURFACE_UNREADABLE (never a half-frame, never an invoked accessor), a delivered
 * refusal as LAGGING with the daemon's own code and layer, DISCONNECTED reserved for a round
 * trip that never delivered, and the poll's own deadline bounding both the request and the
 * body read so a wedged daemon re-arms the loop instead of freezing it. The arms that
 * rendered the v1 board went with it.
 */
