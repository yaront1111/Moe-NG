...
export function codeMission(
  ...
  const lines = [
    ...
    ...(nodeRef.startsWith("node:v1:") ? [
      ...
      `Your node's acceptance gate is exactly: every acceptance criterion listed for your node, and the verification command \`${brief.test}\`, which the daemon's independent verifier reruns. Nothing else is a required check for this node.`,
      "Other repository checks - CI workflow steps, phase-exit or release scripts, checks for other nodes' work - are context, not your gate: run them when useful and never delete, weaken or skip them.",
      "When such a check fails only because a deliverable another node of your sealed plan owns is missing, record it as an attributed finding: add attributedTo {\"nodeKey\": \"<owning node key>\", \"criterionIds\": [\"<criterion IDs that node owns>\"]} from the sealed-plan map. The daemon records it for that node and the operator; it does not count against your node.",
      "Never attribute a failure of your own criteria, your verification command or your own node: the daemon refuses an attribution naming your node, citing your criteria, or naming a node outside your plan. With an UNKNOWN map, report the failure as your own finding instead.",
      ...
      "An UNKNOWN or incomplete map is missing context, not evidence of a plan conflict. Preserve the repository's checks and the daemon's independent verifier.",
