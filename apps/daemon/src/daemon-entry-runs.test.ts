...
async function post(
  ...
  return new Promise((resolve, reject) => {
    const request = httpRequest({
      ...
      host: "127.0.0.1", method: "POST", path: "/runs/read", port: started.port, setHost: false,
    }, (response) => {
      const chunks: Buffer[] = [];
...
it("resolves and forwards the project-bound runs reader", async () => {
  const seen: unknown[] = [];
  const started = await startDaemon({
    ...
    dependencies: {
      ...
      runs: () => ({
        ...
        readRuns: (selector: unknown) => {
          seen.push(selector);
          return { code: "RUNS_READ_GOAL_UNKNOWN", layer: "TEST_READER", outcome: "REFUSED" as const };
        },
      }),
    },
  });
  ...
  try {
    expect(await post(started, JSON.stringify({ goalRef: "goal-entry" }))).toEqual({
      body: { code: "RUNS_READ_GOAL_UNKNOWN", layer: "TEST_READER", outcome: "REFUSED" }, status: 200,
    });
...
it("refuses the runs route as unavailable when the provider offers no reader", async () => {
  const started = await startDaemon({
    csrfToken: CSRF,
    dependencies: {
      provide: () => ({ ...fixtureDependencies(), authenticator: authenticator([CAPABILITIES.GOAL]) }),
  ...
  if (!started.ok) throw new Error(`daemon failed: ${started.code}`);
  try {
    expect(await post(started, "{}")).toMatchObject({ body: { code: "LISTENER_RUNS_UNAVAILABLE" }, status: 503 });
  } finally {
    await started.shutdown();
  }
});
