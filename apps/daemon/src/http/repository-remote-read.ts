...
import { decodePublicationCandidate, type PublicationApproval, type PublicationCandidateReader } from "../repository/publication-approval-contracts.js";
...
export function createRepositoryRemoteReadPort(
  ...
  const preparePublication = (goalId: string, raw: string | null): PublicationCandidateView => {
    try {
      ...
      const measured = options.readPublicationCandidate?.(remote, { goalId, remoteDefaultBranch: readRemoteDefaultBranch(store, projectId, remote) });
