...
const { execute, restorePaths } = vi.hoisted(() => ({
  execute: vi.fn(), restorePaths: [] as string[],
}));
vi.mock("node:fs", async (original) => {
  ...
  return {
    ...
    mkdtempSync: (...args: Parameters<typeof actual.mkdtempSync>) => {
      ...
      if (typeof path === "string" && basename(path).startsWith("moe-backup-restore-")) {
...
afterEach(() => {
  vi.useRealTimers();
  execute.mockReset();
  for (const path of restorePaths.splice(0)) rmSync(path, { recursive: true, force: true });
...
describe("restore-container readiness", () => {
  it("waits for the final TCP server, not the temporary socket-only initialization server", async () => {
    ...
    expect(restorePaths).toHaveLength(1);
    expect(restorePaths.map((path) => existsSync(path))).toEqual([false]);
  });
