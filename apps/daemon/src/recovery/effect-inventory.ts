...
import type { RecoveryReconciliationRecorded } from "./recovery-inventory-ledger.js";
...
export async function reconcileEffectInventory(
  ...
  const written = recordRecoveryReconciliation(store, {
  ...
  if (read.record.recordDigest !== built.record.recordDigest) {
