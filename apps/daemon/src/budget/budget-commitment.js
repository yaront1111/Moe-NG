...
export const BUDGET_COMMITMENT_LAYER = "DAEMON_PREREQUISITE";
 ...
 */
export const GENESIS_AMOUNTS = Object.freeze(NODE_ADMISSION_METERS.map((meter) => Object.freeze({ amount: 0, meter })));
const DOMAIN = "moe/budget/commitment/v1";
const HEX64 = /^[0-9a-f]{64}$/u;
const ENCODER = new TextEncoder();
const refuse = (code, refusal) => Object.freeze({
    code, layer: BUDGET_COMMITMENT_LAYER, ok: false, refusal,
    upstream: Object.freeze({ code: refusal.code, layer: refusal.layer }),
});
const deny = (code) => Object.freeze({ code, layer: BUDGET_COMMITMENT_LAYER, ok: false });
/**
...
function canonicalText(value) {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new TypeError("budget commitment: non-finite number");
        return JSON.stringify(value);
    }
 ...
 */
export function budgetCommitmentMaterial(store, input) {
    const bound = genesisBudgetBindingPort(input.approvedRun)(store, input.projectId, input.goalRef);
    if (!bound.ok) {
 ...
 */
export function budgetCommitmentDigest(material) {
    const ordered = {};
    for (const key of BUDGET_COMMITMENT_MATERIAL_KEYS)
        ordered[key] = material[key];
 ...
 */
export function verifyBudgetCommitment(store, input, expectedRef) {
    ...
    if (!built.ok)
        return deny("BUDGET_COMMITMENT_MATERIAL_UNAVAILABLE");
