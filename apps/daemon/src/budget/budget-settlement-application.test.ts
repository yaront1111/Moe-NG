...
import { activateBudgetReservation } from "./budget-ledger-holds.js";
import {
  ATTEMPT,
  GOAL_ID,
  PRINCIPAL_ID,
...
/** A read-only fault seam over the real store; every method not named stays bound to it. */
function overrideStoreMethod(
  store: SqliteEventStore,
  property: string,
  replacement: (...args: unknown[]) => unknown,
): SqliteEventStore {
  return new Proxy(store, {
    get(target, heldProperty): unknown {
      const held: unknown = Reflect.get(target, heldProperty, target);
      if (typeof held !== "function") return held;
      if (heldProperty === property) return replacement;
...
describe("provider usage applied to budget — first refusal rungs", () => {
  it.each([
    ["", "BUDGET_SETTLEMENT_REQUEST_INVALID", null, null],
    [
      "attempt-never-run", "BUDGET_SETTLEMENT_RUN_ABSENT",
      "PROVIDER_RUN_EVIDENCE_ABSENT", "PROVIDER_RUN_READER",
    ],
  ] as const)("refuses %s before any durable write", (attemptRef, code, sourceCode, sourceLayer) => {
    const store = openUnactivatedBudgetFixture(`first-rung-${code}`);
    try {
      expect(storeWideEventHorizon(store)).toBe(0n);
      expect(refusalOf(apply(store, attemptRef))).toStrictEqual({
        code, layer: "BUDGET_SETTLEMENT_APPLICATION", sourceCode, sourceLayer,
      });
      expect(storeWideEventHorizon(store)).toBe(0n);
    } finally {
      store.close();
...
describe("provider usage applied to budget — later refusal rungs", () => {
  it("rung 3: goal unresolved after a durable provider run", () => {
    const store = activatedStore("rung-goal-unresolved");
    const held = heldOf(store);
    commitRun(store, held.attemptRef, []);
    const before = ledgerHeadOf(store);
    // The underlying store still carries the run and graph. This read seam makes the CURRENT
    // graph enumeration unavailable only to this invocation, after the run has read back.
    const noCurrentGraph = overrideStoreMethod(
      store, "enumerateAggregateIdsByPrefix", () => [],
    );

    expect(refusalOf(apply(noCurrentGraph, held.attemptRef))).toStrictEqual({
      code: "BUDGET_SETTLEMENT_GOAL_UNRESOLVED",
  ...
  it("rung 4: budget ledger unreadable after a durable run and goal", () => {
    const store = activatedStore("rung-ledger-unreadable");
    const held = heldOf(store);
    commitRun(store, held.attemptRef, []);
    const before = ledgerHeadOf(store);
    const readEvents = store.readEvents.bind(store);
    // The active graph still resolves the durable goal. Withhold only that goal's ledger-binding
    // row from the subsequent budget read, so this reaches rung 4 rather than collapsing to 3.
    const noGoalBinding = overrideStoreMethod(store, "readEvents", (...args: unknown[]) =>
      args[0] === GOAL_ID ? [] : readEvents(String(args[0])));

    expect(refusalOf(apply(noGoalBinding, held.attemptRef))).toStrictEqual({
      code: "BUDGET_SETTLEMENT_LEDGER_UNREADABLE",
      layer: "BUDGET_SETTLEMENT_APPLICATION",
      sourceCode: "BUDGET_PROJECTION_GOAL_ABSENT",
      sourceLayer: "BUDGET_CURRENT_PROJECTION",
    });
    expect(ledgerHeadOf(store)).toBe(before);
  });

  it("rung 5: ambiguous activated reservations for one attempt", () => {
    const store = activatedStore("rung-reservation-ambiguous");
    const held = heldOf(store);
    // A second real ingress mints its reservation, while the injected budget-store refusal leaves
    // that hold RESERVED. Drive the production transition writer over that durable hold with the
    // first attempt ref to construct the hostile double-bind this guard must reject.
    const second = runEffectActivateCommand(refuseBudgetCommit(store), secondActivateBytes());
    expect(second).toMatchObject({ ok: true });
    const current = readCurrentBudgetLedger(store, PROJECT_ID, GOAL_ID);
    if (!current.ok) throw new Error(`the ledger must read back: ${current.code}`);
    const reserved = current.reservations.find((entry) => entry.state === "RESERVED");
    if (reserved === undefined) throw new Error("the second ingress must leave a reserved hold");
    acceptedOf(activateBudgetReservation(store, {
      attemptRef: held.attemptRef,
      context: {
        commandId: "cmd-bind-ambiguous", correlationId: "corr-bind-ambiguous",
        decidedAt: "2026-08-15T00:00:01.000Z", principalId: PRINCIPAL_ID,
      },
      goalRef: GOAL_ID,
    ...
    expect(refusalOf(apply(store, held.attemptRef))).toStrictEqual({
      code: "BUDGET_SETTLEMENT_RESERVATION_AMBIGUOUS",
      layer: "BUDGET_SETTLEMENT_APPLICATION",
      sourceCode: null,
      sourceLayer: null,
    });
  ...
  it("rung 6: activated reservation absent after all earlier evidence exists", () => {
    const store = readyStore("rung-reservation-absent");
    const activated = runEffectActivateCommand(refuseBudgetCommit(store), activateBytes());
    expect(activated).toMatchObject({ ok: true });
    commitRun(store, ATTEMPT, [], "cmd-run-rung-6");
    const before = ledgerHeadOf(store);

    expect(refusalOf(apply(store, ATTEMPT))).toStrictEqual({
      code: "BUDGET_SETTLEMENT_RESERVATION_ABSENT",
      layer: "BUDGET_SETTLEMENT_APPLICATION",
      sourceCode: null,
      sourceLayer: null,
    });
...
describe("provider usage applied to budget — UNKNOWN coverage", () => {
  ...
  it("COMPLETE, PARTIAL, and UNKNOWN settlement dispositions remain distinct", () => {
...
describe("provider usage applied to budget — binding refusals move no money", () => {
  ...
  it("activation ingress reaches the budget-binding store refusal", () => {
...
describe("provider usage applied to budget — two attempts, one ledger", () => {
  it("settlement replay and cross-attempt binding preserve conserved balances", () => {
    ...
    // A second application cannot spend the same measured receipt twice. COMPLETE prunes the
    // resolved pair, so the replay answers RESERVATION_ABSENT while the durable position stays
    // byte-for-byte conserved.
    expect(refusalOf(apply(store, held.attemptRef)).code)
      .toBe("BUDGET_SETTLEMENT_RESERVATION_ABSENT");
    expect(ledgerHeadOf(store)).toBe(afterFirst);
    expect(meterViewOf(store, meter)).toEqual(viewAfterFirst);
