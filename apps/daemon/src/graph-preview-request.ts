/**
 * The advisory graph-preview request boundary.
 *
 * Extracted VERBATIM from the package barrel, which had grown to 389 physical
 * lines with this behavior block inside it. A barrel that also decides something
 * cannot be kept under the per-file target while the surface it publishes grows,
 * and behavior living in the entry file is invisible to anyone reading for it.
 *
 * Every symbol here is re-exported from `./index.ts` under its original name, so
 * no consumer's import path changes: this move is a relocation, not a behavior
 * change, and `graph-preview-request.test.ts` still reaches it through the root.
 */
