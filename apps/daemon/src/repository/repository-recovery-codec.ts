...
export function decodeRepositoryRecoveryPayload(value: unknown): RepositoryRecoveryPayload | null {
  ...
  const expected = descriptors["action"]?.value === "RESUME_REVIEW" ? [...keys, "expectedReviewVersion", "expectedReviewDigest"] : keys;
  ...
  if ((row["action"] !== "ABORT_UNEXECUTED" && row["action"] !== "RECONCILE_LANDED" && row["action"] !== "RESUME_REVIEW") || row["decision"] !== "APPROVE"
  ...
  if (row["action"] === "RESUME_REVIEW" && (typeof row["expectedReviewVersion"] !== "number"
  ...
  return Object.freeze({ action: row["action"], decision: row["decision"], expectedReservationRevision: row["expectedReservationRevision"],
    nodeRef: row["nodeRef"], reason: row["reason"], ...(row["action"] === "RESUME_REVIEW"
