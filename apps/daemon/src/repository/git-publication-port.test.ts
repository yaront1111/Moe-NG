...
describe("immutable publication Git port", () => {
  ...
  it("tells a push git refused (a numeric exit) from one that never answered (a null code or a throw)", async () => {
    const answers = new Map<number | null | "throw", unknown>([[0, { ok: true }],
      [1, { ok: false, code: "PUBLISH_PUSH_REJECTED", detail: "PUBLISH_PUSH_REJECTED" }],
      [128, { ok: false, code: "PUBLISH_PUSH_REJECTED", detail: "PUBLISH_PUSH_REJECTED" }],
      [null, { ok: false, code: "PUBLISH_PUSH_UNKNOWN", detail: "PUBLISH_PUSH_UNKNOWN" }],
      ["throw", { ok: false, code: "PUBLISH_PUSH_UNKNOWN", detail: "PUBLISH_PUSH_UNKNOWN" }]]);
    for (const [exit, answer] of answers) {
      const f = fixture(undefined, exit);
      expect(await f.port.push(candidate)).toEqual(answer);
