...
- The initial root diagnostic ran 430 files and 10,378 tests while concurrent writers changed the tree. It finished with 14 failed tests and three failed suites. Its packaging hash-boundary and workspace-capture timeout cases pass in isolation; the scheduler boundary, dependency scanner, compiled-product fixture, runner ABI and portability failures pass after the respective fixes/reruns. This mixed-state run is not a green final baseline.
...
Legacy plans and compiler paths without the immutable contract binding remain unattributable. The separate real-product browser-to-release demonstration still awaits the selected PRD; none of the fixture evidence above is presented as that release.
