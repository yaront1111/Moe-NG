...
- **Review ceiling (SmartBear/Cisco).** Reviewer defect-discovery collapses past ~200–400 changed LOC, so prefer plans that land in small focused files. This is plan-shaping guidance, NOT a QA rejection bar: task-level net LOC is never grounds to reject. What QA does enforce is the per-PRODUCTION-FILE cap (target ≤250, reject >400).
...
| Net changed LOC per PRODUCTION FILE | ≤250 | — | >400 (QA rejects; split the FILE) |
