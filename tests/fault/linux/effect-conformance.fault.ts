 ...
 *
 * AUTHORITY: only a real Linux host can make this file say anything about
 * Linux. Run anywhere else it asserts the refusal instead — a wrong host is
...
const SLOT = "linux";
const PLATFORM_LAYER = "PLATFORM_LINUX";
...
describe("Linux effect conformance over every frozen platform boundary", () => {
  ...
  it(
    ...
    async () => {
      ...
      if (host !== SLOT) {
        ...
        if (run.ok) {
          throw new Error("a non-Linux host must not produce a Linux schedule run");
        }
