...
import {
  admitCargoPackTool, PackCargoToolError, readCargoToolchainPins,
...
import {
  PACK_TOOLCHAIN_SCHEMA, type WindowsPackToolchain,
} from "./pack-toolchain-codec.js";
...
export {
  PACK_TOOLCHAIN_SCHEMA, parseWindowsPackToolchain, serializeWindowsPackToolchain,
} from "./pack-toolchain-codec.js";
export type { WindowsPackToolchain } from "./pack-toolchain-codec.js";

const PINS = readToolchainPins();
const CARGO_PINS = readCargoToolchainPins();
...
export function resolveCargoPackTool(
  ...
  return admitCargoPackTool(repositoryRoot, explicitExecutable, CARGO_PINS);
}

/** Pinned rustup toolchain only; never an ambient PATH lookup and never a repository path. */
function pinnedCargoExecutable(environment: NodeJS.ProcessEnv): string {
  const profile = environment["USERPROFILE"];
  const rustup = environment["RUSTUP_HOME"]
    ?? (typeof profile === "string" && profile !== "" ? join(profile, ".rustup") : undefined);
  if (typeof rustup !== "string" || rustup === "" || !isAbsolute(rustup)) {
    throw new PackCargoToolError();
  }
  return join(rustup, "toolchains", CARGO_PINS.toolchain, "bin", "cargo.exe");
}
...
export function resolveWindowsPackToolchain(
  ...
  const cargo = resolveCargoPackTool(repositoryRoot,
    dependencies.cargoExecutable ?? pinnedCargoExecutable(environment));
  return Object.freeze({ cargo, node, pnpm, powershell, schemaVersion: PACK_TOOLCHAIN_SCHEMA });
}
